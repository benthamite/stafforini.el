...
1. =M-x stafforini-export-all-notes= (also runs inject-lastmod, backlinks, citing-notes).
2. =M-x stafforini-export-all-quotes= (also runs id-slug-map, work pages, topic pages).
3. =M-x stafforini-start-server=.
