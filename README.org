...
1. Edit the =.bib= file.
2. =M-x stafforini-update-works=.
3. =M-x stafforini-start-server= — *restart required* (Hugo doesn't re-render note pages that cite the changed work).
...
1. =M-x stafforini-update-backlinks=.
2. =M-x stafforini-start-server= — *restart required* (backlinks data affects other pages).
