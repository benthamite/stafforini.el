...
=M-x stafforini-deploy-fast-note= (or =f= in the menu) runs the fast note deploy path (passing =--fast-note= to =deploy.sh=). Use this only for minor already-exported note body edits. It preserves the existing =public/= snapshot and Pagefind search index, so use the normal deploy path for slug, date, tag, citation, link, template, quote, work-page, PDF, new-note, or deleted-note changes.
...
| =M-x stafforini-deploy-fast-note=  | Deploy a minor already-exported note body edit         |
