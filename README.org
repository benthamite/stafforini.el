...
With a prefix argument (=C-u M-x stafforini-deploy=, or =C-u D= in the menu), the command runs a quick deploy that skips data regeneration and PDF processing.  Use this when only templates or styles changed.
