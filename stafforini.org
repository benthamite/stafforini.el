...
#+findex: stafforini-deploy-fast-note
...
=deploy.sh=.  With a prefix argument (=C-u M-x stafforini-deploy=), the
command passes =--quick= to =deploy.sh=, which skips content export,
data regeneration, and PDF processing.  Use the quick form when only
templates, styles, or static assets changed after a recent full export.

The command ~stafforini-deploy-fast-note~ exports the current note and
deploys a minor note body edit.  Internally it runs ox-hugo for the
current note and then passes =--fast-note= to =deploy.sh=, preserving the
existing =public/= snapshot and Pagefind search index.  Use the normal
deploy path for slug, date, tag, citation, link, template, quote,
work-page, PDF, new-note, or deleted-note changes.
...
- *Generate*: ~stafforini-publish-note~ (=p=),
  ~stafforini-update-works~ (=w=), ~stafforini-update-backlinks~ (=b=),
  and ~stafforini-process-pdfs~ (=d=).
- *Auxiliary*: ~stafforini-generate-id-slug-map~ (=m=),
  ~stafforini-generate-topic-pages~ (=t=),
  ~stafforini-generate-citing-notes~ (=c=), and
  ~stafforini-inject-lastmod~ (=l=).
...
- *Insert*: ~stafforini-insert-image~ (=I=) and
  ~stafforini-insert-topics~ (=T=).
