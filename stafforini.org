...
The command ~stafforini-deploy-fast-note~ deploys a minor
already-exported note body edit.  Internally it passes =--fast-note= to
=deploy.sh=, preserving the existing =public/= snapshot and Pagefind
search index.  Use the normal deploy path for slug, date, tag,
citation, link, template, quote, work-page, PDF, new-note, or
deleted-note changes.
